...
import os
import threading
import copy  # Added to help with object copying
...
def interpolate_transforms(trans1, trans2, steps=100):
...
def solveAngle(transforms):
    returnAngles = []
    zeroAngle = [0,0,0,0,0,0]

    for i, trans in enumerate(transforms):
        try:
            tempAngle = ARM6_kinematics_inverse_arm(trans,zeroAngle)
...
def inter_solve(trans1,trans2,NUM_STEP) :
    ...
    solvedAngle = solveAngle(transforms,NUM_STEP)
